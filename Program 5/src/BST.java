...
public class BST<Key extends Comparable<Key>, Value> {
    ...
    private Queue<Key> inOrder(Node x)
    {
    	...
    	Queue<Key> left = inOrder(x.left);
    	Queue<Key> right = inOrder(x.right);
    ...
    private Pair maxImbalance(Node x) //TODO: NOT WORKING
    {
    	...
    	{
    		Pair right = maxImbalance(x.right);
    		return new Pair(right.height+1, right.height+1);
    	}
    ...
    {
    	//create a new BST
    	BST<Key, Value> reversedTree = new BST<Key, Value>();
    	if (root != null) reversedTree.root = new Node(null, null, 0);
    	
    	//Copy elements to new BST, reversing them
    	reverse(root, reversedTree.root);
    ...
    private void reverse(Node originalNode, Node newNode)
    {
    	...
    	//copy information to new tree
    	newNode.key = originalNode.key;
    	newNode.val = originalNode.val;
    	newNode.size = originalNode.size;
    	
    	//create children nodes if necessary
    	if (originalNode.right != null) newNode.left = new Node(null, null, 0);
    	if (originalNode.left != null) newNode.right = new Node(null, null, 0);
    	
    	//copy children
    	reverse(originalNode.right, newNode.left);
    ...
    private int isSymmetric(Node x)
    {
    	...
    	if ((x.left == null && x.right != null) ||
    	   (x.left != null && x.right == null)) return -1;
    	int height = isSymmetric(x.left);
    	
    	if (height == isSymmetric(x.right) && height != -1) return height+1;
